...
colnames(animacyMatrix) <- c(1:92) # Rename column names to the correct number for easier extraction later
...
selectSameAnimacy <- function(df, animacy) {
  ...
  m <- matrix(cells, nrow = rows, byrow = TRUE)
  df <- as.data.frame(m)
  colnames(df) <- c("x","y","animacystate","dissimilarity")
  return(df)
}
