...
aniface0in0x0<- as.data.frame(matrix(0, 92,92))
colnames(aniface0in0x0) <- c(1: ncol(aniface0in0x0))

# ---> dit weer in een 92x92 raster zetten met de rest op 0 
...
#Create dataframe  with column #1 original 

anova_test <- aov(as.matrix(anova_neur) ~ as.matrix(anova_aniface0) + as.matrix(anova_animacy0))
summary(anova_test)

anova_test1 <- aov(as.vector(neur) ~ as.vector(anova_aniface0) + as.vector(anova_animacy0))
summary(anova_test1)


inputframe <- data.frame(diss = as.vector(anova_neur), a = as.factor(as.vector(anova_animacy0)), af = as.factor(as.vector(anova_aniface0)))
inputframe
...
typeof(neur)
typeof(anova_aniface0)


saveRDS(neur, "neur.rds")
saveRDS(aniface0, "aniface0.rds")
saveRDS(animacy0, "animacy0.rds")
